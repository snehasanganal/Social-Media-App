...
class PrivacyBERTLSTM(torch.nn.Module):
    def __init__(self, learning_rate, batch_size, optimizer_choice):
        ...
        self.bert = BertModel.from_pretrained(TOKENIZER).to(self.device)  # Move BERT to device
        ...
        self.fc = torch.nn.Linear(HIDDEN_SIZE * 2, 2).to(self.device)  # Binary classification (Sensitive/Non-Sensitive)
    ...
    def fit(self, train_data):
        ...
        model_path = f"/content/binary_model_lr_{self.batch_size}_{self.optimizer.__class__.__name__}.pt"
